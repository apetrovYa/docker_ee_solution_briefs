..  _elk_ensure_sufficient_threads:
...
In a production environment it is important to ensure that increased `ulimits <https://kerneltalks.com/linux/ulimit-value-need-know/>`_ for **nproc** (number of processes) are available for
the Elasticsearch services. This can be configured by setting the **nproc** configuration parameter on the
